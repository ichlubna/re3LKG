...
This repository is an extension of the original RE3 repo. This code supports multiview rendering which can be used to play GTA VC on a 3D display by Looking Glass Factory. Currently, only OpenGL and GTA VC support is implemented. The following parameters need to be added to the gtaVC.ini file. The parameters are calibration values for the given 3D display and can be retreived by using [this](https://github.com/ichlubna/getLKGCalibration) tool. The numpad keys 7 and 8 are used to refocus the scene and 2 and 3 to adjust the distance between the cameras and the 3D feel. The rows, cols and steps can be adjusted independently on the display model.
